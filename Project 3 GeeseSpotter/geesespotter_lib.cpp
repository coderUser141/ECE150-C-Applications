...
char* createBoard(std::size_t xdim, std::size_t ydim) {
    char* board = (char*)std::calloc(xdim * ydim, sizeof(char));
    return board;
}
...
void printBoard(char* board, std::size_t xdim, std::size_t ydim) {
    ...
    for (int i = 0; i < xdim * ydim; ++i) {
        ...
        if ((board[i] ^ 16) < 16) {
            printedBoard[i] = (board[i] ^ 0x00110000);
        }
    }

    for (int i = 0; i < xdim; ++i) {
        for (int j = 0; j < ydim; ++j) {
            std::cout << printedBoard[j];
        }
        std::cout << std::endl;
    }
...
void computeNeighbors(char* board, std::size_t xdim, std::size_t ydim) {
    for (int i = 0; i < xdim * ydim; ++i) {
        if (i != 9) {
            if (board[i + 1] == 9)board[i]++;
            if (board[i - 1] == 9)board[i]++;
            if (board[i + xdim] == 9)board[i]++;
            if (board[i - xdim] == 9)board[i]++;
            if (board[i + xdim + 1] == 9)board[i]++;
            if (board[i + xdim - 1] == 9)board[i]++;
            if (board[i - xdim - 1] == 9)board[i]++;
            if (board[i - xdim + 1] == 9)board[i]++;
        }
...
int reveal(char* board, std::size_t xdim, std::size_t ydim, std::size_t xloc, std::size_t yloc) {
    int boardLocation = yloc * xdim + (xloc - xdim); //calculates the position in array

    if (~board[boardLocation] == 255) {
        if (!(board[boardLocation + 1] ^ 32))board[boardLocation + 1] -= 32;
        if (!(board[boardLocation - 1] ^ 32))board[boardLocation - 1] -= 32;
        if (!(board[boardLocation + xdim] ^ 32))board[boardLocation + xdim] -= 32;
        if (!(board[boardLocation - xdim] ^ 32))board[boardLocation - xdim] -= 32;
        if (!(board[boardLocation + xdim + 1] ^ 32))board[boardLocation + xdim + 1] -= 32;
        if (!(board[boardLocation + xdim - 1] ^ 32))board[boardLocation + 1 - xdim] -= 32;
        if (!(board[boardLocation - xdim - 1] ^ 32))board[boardLocation - 1 + xdim] -= 32;
        if (!(board[boardLocation - xdim + 1] ^ 32))board[boardLocation - 1 - xdim] -= 32;
    }

    if (board[boardLocation] & 16) {
        return 1;
    } else if (board[boardLocation] & 32) {
        return 2;
    } else if (board[boardLocation] & 8) {
        return 9;
    } else {
        return 0;
    }
